...
public class PaymentController {
	...
	public static void addPayment() {
		...
		addtopay(new Payment(guest_id, reservation_id,add_on, room_price, total_nights,total_charges,payment_status));
		System.out.println("Guest has been registered successfully");

	}
	public static void addtopay(Payment g) {
		try {
			Statement statement = DbUtilities.getConnection().createStatement();
			String sqlQuery="insert into tbl_payment(Guest_id,Reservation_id,Add_on,Room_price,Number_of_nights,Total_charges,Payment_status) values(?,?,?,?,?,?,?)";
			...
			pstm.setInt(2,g.getReservation_id());
			pstm.setString(3, g.getAdd_on());
			pstm.setInt(4,g.getRoom_price());
			pstm.setInt(5,g.getNumber_of_nights());
			pstm.setInt(6,g.getTotal_charges());
			pstm.setInt(7,g.getPayment_status());
			pstm.addBatch();
